...
from flowDet_mmdet.utils.gmmDet_utils import SamplingDetector
from flowDet_mmdet.flowDet_env import FLOWDET_FEAT_ROOT
...
for i in range(num_models):
    file = f'{save_dir}/{args.saveNm}{i+1}.json'
    try:
        with open(file, 'r') as f:
            allOutputs[i] = json.load(f)
            print(f"Loaded {file}")
    except:
        print(f'Missing results file for {file}.json')
        exit()
